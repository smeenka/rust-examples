...
use core::fmt::Write;
use hal::i2c::{Config, SlaveAddressMask, I2cDirection, I2cSlave, Error};
...
fn main() -> ! {
    ...
    let mut usart = dp
        ...
        .unwrap();
    writeln!(usart, "i2c slave test. Should be used together with i2c_master\r").unwrap();
    ...
    loop {
        ...
        for i in 0..buf_long.len() {buf_long[i] = 0x41 + (i as u8)};
        ...
        match address {
            ...
            // 0x41 good case master write slave read: master does send 20 bytes slave receives 20 bytes
                ...
                match i2c.slave_read( &mut buf_short) {
                    ...
                    Err(err) => {errors +=1; writeln!(usart, "Test 0x41 failed. Error: {:?}\r", err).unwrap()},
                };
            },
            0x42 => {
                ...
                match i2c.slave_read( &mut buf_short) {
                    Ok(_)  => {
                        {errors +=1; writeln!(usart, "Test 0x42 failed. should return error IncorrectFrameSize").unwrap()};
                    },
                    Err(err) => match err {
                        Error::IncorrectFrameSize(n) => writeln!(usart, "Test 0x42 Ok Received expected IncorrectFrameSize bytes: {}\r", n).unwrap(),
                        _ => {errors +=1; writeln!(usart, "Test 0x42 failed. Error: {:?}\r", err).unwrap() }
                    }
                };
            },
            0x43 => {
                ...
                match dir {
                    I2cDirection::MasterWriteSlaveRead => {
                        ...
                        match i2c.slave_read( &mut buf_short) {
                            ...
                            Err(err) => {errors +=1; writeln!(usart, "Test 0x44 recv failed. Error: {:?}\r", err).unwrap()},
                        };
                    ...
                    I2cDirection::MasterReadSlaveWrite => {
                        ...
                        match i2c.slave_write( &buf_short) {
                            ...
                            Err(err) => {errors +=1; writeln!(usart, "Test 0x44 send failed. Error: {:?}\r", err).unwrap()},
                        };
                    },
                    _ => {writeln!(usart, "UU\r").unwrap();}
                };
            },
            0x48 => {
                ...
                match i2c.slave_write( &buf_short) {
                    ...
                    Err(err) => {errors +=1; writeln!(usart, "Test 0x48 failed. Error: {:?}\r", err).unwrap()},
                };
            },
            0x49 => {
            ...
            0x4A => {
                ...
                match i2c.slave_write( &buf_long) {
                    Ok(_)  => {
                        errors += 1;
                        writeln!(usart, "Test 0x4A should fail with IncorrectFrameSize error\r").unwrap();
                    },
                    Err(err) => match err {
                        Error::IncorrectFrameSize(n) => writeln!(usart, "Test  0x4A Ok Received expected IncorrectFrameSize bytes: {}\r", n).unwrap(),
                        _ => {errors +=1; writeln!(usart, "Test 0x4A failed. Error: {:?}\r", err).unwrap()},
                    }
                }
            }, 
            0x4B => {
            ...
            0x4F => {
                ...
                writeln!(usart, "Test finished. nr tests/nr errors: {}/{}!\r", tcount, errors).unwrap();
                writeln!(usart, "-----\r").unwrap();
                tcount = 0;
                errors = 0;
            },
